...
static uint8_t *s_pd_frame = NULL;
static size_t s_pd_frame_size = 0;
static volatile bool s_pd_busy = false;

static uint32_t s_pd_w = 0, s_pd_h = 0, s_pd_fmt = 0;
...
static void pd_task(void *arg)
{
    while (1)
    {
        ...
        if (s_pd_frame && s_pd_frame_size)
        {
            pd_run_frame(s_pd_frame, s_pd_w, s_pd_h, s_pd_fmt);
        }
        s_pd_busy = false;
    }
}
...
static esp_err_t pd_last_handler(httpd_req_t *req)
{
    pd_result_t r;
    pd_get_last(&r);

    cJSON *root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "count", r.count);
    cJSON_AddNumberToObject(root, "frame_id", r.frame_id);
    cJSON_AddNumberToObject(root, "ts_ms", r.ts_ms);

    cJSON *arr = cJSON_CreateArray();
    cJSON_AddItemToObject(root, "boxes", arr);

    for (int i = 0; i < r.count && i < 8; i++)
    {
        const pd_box_t *b = &r.boxes[i];

        int cx = b->x + b->w / 2;
        int cy = b->y + b->h / 2;

        // normalizados 0..1 (útiles para “seguir” al centro)
        float nx = (s_pd_w > 0) ? ((float)cx / (float)s_pd_w) : 0.0f;
        float ny = (s_pd_h > 0) ? ((float)cy / (float)s_pd_h) : 0.0f;

        cJSON *o = cJSON_CreateObject();
        cJSON_AddNumberToObject(o, "x", b->x);
        cJSON_AddNumberToObject(o, "y", b->y);
        cJSON_AddNumberToObject(o, "w", b->w);
        cJSON_AddNumberToObject(o, "h", b->h);
        cJSON_AddNumberToObject(o, "score", b->score);

        cJSON_AddNumberToObject(o, "cx", cx);
        cJSON_AddNumberToObject(o, "cy", cy);
        cJSON_AddNumberToObject(o, "nx", nx);
        cJSON_AddNumberToObject(o, "ny", ny);
...
static esp_err_t image_stream_handler(httpd_req_t *req)
{
    ...
    while (1)
    {
...

        {
            ...
            size_t need = (size_t)s_pd_w * (size_t)s_pd_h * 2;

            if (s_pd_frame == NULL)
            {
                s_pd_frame_size = need;
                s_pd_frame = (uint8_t *)heap_caps_malloc(s_pd_frame_size, MALLOC_CAP_SPIRAM);
                ESP_LOGI("PD", "Allocated PD copy buffer: %u bytes", (unsigned)s_pd_frame_size);
            }
...
static esp_err_t http_server_init(web_cam_t *web_cam)
{
    ...
    // ---- Aliases para compatibilidad con el front ----
    httpd_uri_t camera_info_uri_alias = {
        .uri = "/api/camera_info",
        .method = HTTP_GET,
        .handler = camera_info_handler,
        .user_ctx = (void *)web_cam};

    httpd_uri_t camera_settings_uri_alias = {
        .uri = "/api/camera_settings",
        .method = HTTP_POST,
        .handler = camera_settings_handler,
        .user_ctx = (void *)web_cam};
    httpd_uri_t pd_last_uri = {
        .uri = "/api/pd_last",
        .method = HTTP_GET,
        .handler = pd_last_handler,
        .user_ctx = (void *)web_cam};
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(stream_httpd, &pd_last_uri);
        httpd_register_uri_handler(stream_httpd, &camera_info_uri_alias);
        httpd_register_uri_handler(stream_httpd, &camera_settings_uri_alias);
...
void app_main(void)
{
   ...
   // turret_home();
    ...
    // Aquí todavía no sabemos el buffer_size real, lo asignamos luego dinámico
    // pero si quieres, puedes asignar el máximo esperado.
    // (lo más seguro es asignarlo en el primer frame del stream, te lo dejo abajo)

    xTaskCreatePinnedToCore(pd_task, "pd_task", 8192, NULL, 5, &s_pd_task, 1);
